import os
import logging
import pandas as pd
from datetime import datetime
from typing import Dict, Any

log = logging.getLogger(__name__)

class DataWriter:
    """
    Responsável por salvar os relatórios em arquivos (local ou Colab).
    """
    def __init__(self, config: object, gdrive_service: Any = None):
        """
        Recebe o módulo de config para operar.
        """
        self.config = config
        self.mode = config.MODO_EXECUCAO
        self.gdrive_service = gdrive_service
        
        if self.mode == 'local':
            self.output_path = "output"
            if not os.path.exists(self.output_path):
                os.makedirs(self.output_path)
        else: 
            self.output_path = self.config.CAMINHOS['colab']['id_pasta_saida']

    def save_report_to_excel(self, report_tabs: Dict[str, pd.DataFrame], base_filename: str) -> str:
        """
        Salva as abas do relatório em um único arquivo Excel.
        Retorna o caminho (local) ou o link (colab).
        """
        
        try:
            ano = self.config.ANO_DO_RELATORIO
            mes = self.config.MES_DO_RELATORIO
            analysis_period = f"{ano:04d}-{mes:02d}"
        except Exception:
            analysis_period = "Analise" 

        time_version = datetime.now().strftime("%Hh%Mm") 
        
        filename = f"{base_filename}_{analysis_period}_{time_version}.xlsx"
        
        if self.mode == 'local':
            return self._save_local(report_tabs, filename)
        else:
            return self._save_to_drive(report_tabs, filename)

    def _save_local(self, report_tabs: Dict[str, pd.DataFrame], filename: str) -> str:
        """Salva o arquivo Excel na pasta 'output' local."""
        full_path = os.path.join(self.output_path, filename)
        
        log.info(f"Iniciando a escrita do relatório em: {full_path}")
        try:
            with pd.ExcelWriter(full_path, engine='xlsxwriter') as writer:
                count = 0
                for sheet_name, df in report_tabs.items():
                    if isinstance(df, pd.DataFrame):
                        df.to_excel(writer, sheet_name=sheet_name, index=False)
                        count += 1
                    else:
                        log.warning(f"Item '{sheet_name}' não é um DataFrame, pulando...")
            
            log.info(f"Relatório salvo com sucesso com {count} abas.")
            return full_path
        
        except Exception as e:
            log.error(f"Falha ao salvar o arquivo Excel em {full_path}: {e}", exc_info=False)
            return ""

    def _save_to_drive(self, report_tabs: Dict[str, pd.DataFrame], filename: str) -> str:
        """Salva o arquivo Excel no Google Drive e retorna o link."""
        
        log.info(f"Iniciando upload para o Google Drive: '{filename}'")
        if not self.gdrive_service:
            log.error("Google Drive Service não foi inicializado. Abortando upload.")
            return ""
            
        try:
            temp_path = f"/content/{filename}"
            with pd.ExcelWriter(temp_path, engine='xlsxwriter') as writer:
                for sheet_name, df in report_tabs.items():
                    if isinstance(df, pd.DataFrame):
                        df.to_excel(writer, sheet_name=sheet_name, index=False)

            from googleapiclient.http import MediaFileUpload
            file_metadata = {
                'name': filename,
                'parents': [self.output_path] 
            }
            media = MediaFileUpload(temp_path, mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
            
            file = self.gdrive_service.files().create(
                body=file_metadata,
                media_body=media,
                fields='id, webViewLink'
            ).execute()
            
            log.info(f"Arquivo salvo com sucesso no Google Drive.")
            return file.get('webViewLink')

        except Exception as e:
            log.error(f"Falha ao salvar no Google Drive: {e}", exc_info=True)
            return ""